...
- `*_flags` - if an enum name has such suffix, it means values act as bit flags and multiple can be active at the same time. Such enumerations also have overloaded operators: `&`, `|`, `^`, `~`, `&=`, `|=`, `^=`.
